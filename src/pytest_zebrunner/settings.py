...
class SettingsLoader:
    ...
    def _put_by_path(settings_dict: dict, path: List[str], value: Any) -> None:
        if len(path) == 1:
            ...
            SettingsLoader._put_by_path(current_dict, path[1:], value)
    ...
    def _get_by_path(settings_dict: dict, path: List[str], default_value: Any = None) -> Any:
        if len(path) == 1:
            ...
            return SettingsLoader._get_by_path(inner_dict, path[1:], default_value)

    def _load_env(self, path_list: List[List[str]]) -> dict:
        ...
        for path in path_list:
            ...
            if env_variable is not None:
                SettingsLoader._put_by_path(settings, path, env_variable)

        return settings

    def _load_yaml(self, path_list: List[List[str]]) -> Dict[str, Any]:
        settings: Dict[str, Any] = {}
        ...
        for setting_path in path_list:
            ...
            setting_value = SettingsLoader._get_by_path(yaml_settings, yaml_path)
            if setting_value is not None:
                SettingsLoader._put_by_path(settings, setting_path, setting_value)
    ...
    def load_settings(self) -> Settings:
        ...
        settings: Dict[str, Any] = {}
        yaml_settings = self._load_yaml(settings_path_list)
        settings = deep_update(settings, yaml_settings)
        env_settings = self._load_env(settings_path_list)
